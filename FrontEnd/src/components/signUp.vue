        ...
        <div class="p-6 space-y-4 md:space-y-6 sm:p-8">
          ...
          <form class="space-y-4 md:space-y-6" @submit.prevent="signUp">
            <div>
              ...
              <span class="text-red-400 text-xs text-end text-right" v-for="error in $v.username.$errors">{{
            ...
            <div>
              <div class="relative border border-gray-300 rounded-lg focus:ring-primary-600 focus:border-primary-600">
                <input maxlength="80" :class="{ 'border-2 border-rose-600 ': $v.password.$error }" id="product"
                ...
                <button type="button" @click="passwordToggle" class="absolute top-1/2 right-3 transform -translate-y-1/2">
              ...
              <div>
                ...
                <div v-if="passwordStrength != -1" id="password-strength">
                  <div :class="bgcolor()" class="  text-xs text-center text-white"
                    :style="{ width: passwordStrength + '%' }" id="password-strength-bar">{{ passwordStrengthText }}</div>
                </div>
              </div>
              <span class="text-red-400 text-xs text-end text-right" v-for="error in $v.password.$errors">{{
            ...
            <div>
              ...
              <span class="text-red-400 text-xs text-end text-right" v-for="error in $v.email.$errors">{{
                CustomValidationMsg(error.$message, "Email address") }}</span>
            </div>
            <button type="submit"
            ...
            <p class="text-sm font-light text-gray-500 dark:text-gray-400">
              Already have an account? <a
                class="font-medium text-primary-600 hover:underline dark:text-primary-500"><router-link to="/login">Log
                  in</router-link></a>
            </p>
